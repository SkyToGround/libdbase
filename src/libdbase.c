...
int libdbase_get_list(detector ***list, int *found) {
  ...
  *found = libdbase_list_serials(serials, LEN);
  ...
  if (*list == NULL) {
    fprintf(stderr,
            "E: libdbase_get_list(): Out of memory when allocating "
            "*det-list\n\tfound was %d\n",
            *found);
    return -ENOMEM;
  }
  ...
  for (k = 0; k < *found; k++) {
    ...
    if ((list[0][k] = libdbase_init(serials[k])) == NULL) {
      printf(
          "E: libdbase_list_init() when opening detector %d with serial %d\n",
          k + 1, serials[k]);
      return -1;
    } else if (_DEBUG)
...
detector *libdbase_init(int dbase_serial) {
  ...
  /* Locate digiBASE */
  if (_DEBUG > 0) {
    printf("Locating %s digibase (%d) on usb bus\n",
           dbase_serial == -1 ? "first" : "", dbase_serial);
  }
  dev_handle = dbase_locate(&serial, dbase_serial, cntx);
  if (dev_handle == NULL) {
    fprintf(stderr, "E: libdbase_init() Unable to find and open digiBASE\n");
    return NULL;
  }
  ...
  if (serial > -1) {
    det->serial = serial;
    if (_DEBUG > 0)
      printf("Found digibase with Serial #: %d\n", det->serial);
  }
...
int libdbase_start(detector *det) {
  if (check_detector(det, "libdbase_start") < 0)
    return -1;
...
int libdbase_stop(detector *det) {
  if (check_detector(det, "libdbase_stop") < 0)
    return -1;
...
int libdbase_gs_on(detector *det) {
  if (check_detector(det, "libdbase_gs_on") < 0)
    return -1;
...
int libdbase_gs_off(detector *det) {
  if (check_detector(det, "libdbase_gs_off") < 0)
    return -1;
...
int libdbase_zs_on(detector *det) {
  if (check_detector(det, "libdbase_zs_on") < 0)
    return -1;
...
int libdbase_zs_off(detector *det) {
  if (check_detector(det, "libdbase_zs_off") < 0)
    return -1;
...
int libdbase_hv_on(detector *det) {
  if (check_detector(det, "libdbase_hv_on") < 0)
    return -1;
...
int libdbase_hv_off(detector *det) {
  if (check_detector(det, "libdbase_hv_off") < 0)
    return -1;
...
int libdbase_set_rtp(detector *det, double real_time_preset) {
  if (check_detector(det, "libdbase_set_rtp") < 0)
    return -1;
...
int libdbase_set_ltp(detector *det, double live_time_preset) {
  if (check_detector(det, "libdbase_set_ltp") < 0)
    return -1;
...
int libdbase_clear_presets(detector *det) {
  if (check_detector(det, "libdbase_clear_presets") < 0)
    return -1;
...
int libdbase_clear_counters(detector *det) {
  if (check_detector(det, "libdbase_clear_counters") < 0)
    return -1;
  int err = 0;
...
int libdbase_set_gs_chans(detector *det, ushort center, ushort width) {
  ...
  if (0 == center || center > det->status.LEN || width == 0 ||
      width > det->status.LEN) {
    fprintf(stderr, "E: unable to set gain stab. channels to: [%d, %d, %d]\n",
            center - width / 2, center, center + width / 2);
    return -1;
  }
...
int libdbase_set_zs_chans(detector *det, ushort center, ushort width) {
  ...
  if (center == 0 || center > det->status.LEN || width == 0 ||
      width > det->status.LEN) {
    fprintf(stderr, "E: unable to set zero stab. channels to: [%i, %hu, %i]\n",
            center - width / 2, center, center + width / 2);
    return -1;
  }
...
int libdbase_set_pw(detector *det, float pulse_width) {
  if (check_detector(det, "libdbase_set_pw") < 0)
    return -1;
...
int libdbase_set_fgn(detector *det, float fg) {
  if (check_detector(det, "libdbase_set_fgn") < 0)
    return -1;
  if (fg < 0.40f || fg > 1.1999f) {
    fprintf(
        stderr,
        "E: fine gain out of range: %0.1f, should be in range: 0.4 < x < 1.2\n",
        fg);
    return -1;
  }
  /*
...
*/
void libdbase_print_file_status(const status_msg *status, const int serial,
                                FILE *fh) {
  if (fh == NULL) {
  ...
  */
  ...
  fprintf(fh,"Running    : %s\n", (det->status.CTRL & ON_MASK) > 0 ? "Yes" :
  "No"); fprintf(fh,"Mode       : %s\n", (det->status.CTRL & MODE_MASK) > 0 ?
  "PHA" : "List"); fprintf(fh,"HV on      : %s\n", (det->status.CTRL & HV_MASK)
  > 0 ? "Yes" : "No"); fprintf(fh,"Gain stab. : %s\n", (det->status.CTRL &
  GS_MASK) > 0 ? "Yes" : "No"); fprintf(fh,"Zero stab. : %s\n\n",
  (det->status.CTRL & ZS_MASK) > 0 ? "Yes" : "No"); fprintf(fh,"MCB Chans. :
  %d\n", det->status.LEN + 1); fprintf(fh,"HV Target  : %d V\n",
  (int)(det->status.HVT * HV_FACTOR)); fprintf(fh,"Pulse width: %.2f us\n",
  GET_PW(det->status.PW)); fprintf(fh,"Fine Gain  : %.5f \n", det->status.FGN /
  GAIN_FACTOR); fprintf(fh,"Live Time Preset  : %.2f s\n", det->status.LTP *
  TICKSTOSEC); fprintf(fh,"Live Time         : %.2f s\n", det->status.LT *
  TICKSTOSEC); fprintf(fh,"Real Time Preset  : %.2f s\n", det->status.RTP *
  TICKSTOSEC); fprintf(fh,"Real Time         : %.2f s\n", det->status.RT *
  TICKSTOSEC); fprintf(fh,"Gain Stab. chans  : [%hu %hu %hu]\n",
        det->status.GSL,
        det->status.GSC,
        det->status.GSU);
...
/*int libdbase_save_status(detector *det, const char* dir){
  ...
  if(dir != NULL)
    ...
    if(err != 0 && errno != EEXIST){
      fprintf(stderr,
              "E: libdbase_save_status() Unable to create directory: %s
(err=%d)\n", path, errno); return -1;
    }
    if(_DEBUG > 0){
      if(errno != EEXIST)
  ...
  */
  ...
  if (_DEBUG > 0)
    printf("Status %s the sanity tests\nCTRL before mod:0x%02x\n",
           err == 0 ? "failed" : "passed", (unsigned char)mod->CTRL);
  /*
...
*/
int libdbase_read_lm_packets(detector *det, pulse *buf, int len, int *read,
                             uint32_t *time) {
  if (check_detector(det, "libdbase_read_lm_packets") < 0)
    return -1;
  if (len <= 0 || read == NULL || time == NULL) {
    if (len <= 0)
      fprintf(
          stderr,
          "E: libdbase_parse_lm_packets(), length must be a positive number\n");
    else if (read == NULL)
      fprintf(stderr,
              "E: libdbase_parse_lm_packets(), read pointer can't be null\n");
    else
      fprintf(stderr,
              "E: libdbase_parse_lm_packets(), time pointer can't be null\n");
    return -1;
  }
  int err, io;
  ...
  if (err < 0 || io != 1) {
    fprintf(stderr,
            "E: libdbase_parse_lm_packets() when requesting spectrum "
            "(written=%d)\n",
            io);
  ...
  if (tmp == NULL) {
    fprintf(stderr, "E: failed to allocate temporary buffer in "
                    "libdbase_read_lm_packets()\n");
    return -ENOMEM;
  }
  ...
  */
  /*err = libusb_bulk_transfer(det->dev, EP_IN, (unsigned char*) tmp, len, &io,
   * S_TIMEOUT);*/
  /* 2012-02-25: modified len to correct nbr of bytes */
  err = libusb_bulk_transfer(det->dev, EP_IN, (unsigned char *)tmp, blen, &io,
                             S_TIMEOUT);
  if (err < 0) {
    fprintf(stderr, "E: when reading list mode data (read %d bytes)\n", io);
    err_str("libdbase_parse_lm_packets()", err);
    fprintf(stderr,
            "If you got an overflow error, try increasing the buffer size\n");
    /* Make a clean exit */
    free(tmp);
    return err;
  }
  ...
  /* Parse additional information as well (amp, time) */
    ...
    for (k = 0; k < io / 4; k++) {
      ...
      /* Timestamp then */
        ...
        buf[r].amp = (uint32_t)0x1FFF;
        /* First time tick after (in worst case) MAX_T us */
        buf[r].time = (uint32_t)(tmp[k] & TS_MASK);
        r++;
      }
    }
  }
...
int libdbase_list_serials(int *serials, int len) {
  if (serials == NULL || len == 0) {
    if (len != 0)
      fprintf(stderr, "E: serials array must be initialized before calling "
                      "libdbase_list_serials()\n");
    return -1;
  }
  /*
  ...
  */
  ...
  for (i = 0; i < cnt; i++) {
    ...
    if (err == 0 && desc.idVendor == VENDOR_ID && desc.idProduct == PROD_ID) {
      ...
      if (found == len) {
        if (i < cnt - 1)
          fprintf(stderr, "W: libdbase_list_serials() not all devices "
