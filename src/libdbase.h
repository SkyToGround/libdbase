...
#ifndef __LIBDBASE_H__
#define __LIBDBASE_H__
...
#define PROD_ID 0x000f   /* digiBASE, we have seen 0x001f also */
...
typedef struct {
  ...
  int32_t last_spec[DBASE_LEN +
                    1]; /* diff spectrum (difference since last readout) */
} detector;
...
#define GET_TIMER(x)                                                           \
  ((((x)->status.TMR & 0xff0000) >> 8) + (((x)->status.TMR & 0xff00) >> 8))
...
/*
  ...
  set to 0.
 */
int libdbase_read_lm_packets(
    detector *det,   /* the detector pointer */
