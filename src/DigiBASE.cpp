...
bool DigiBASE::DetectorIsOK() { return is_ok; }
...
void DigiBASE::GetEvents(std::vector<unsigned int> &amplitude,
                         std::vector<unsigned int> &time) {
  int events;
  int res = libdbase_read_lm_packets(det, pulseBuffer,
                                     sizeof(pulse) * pulseBufferSize, &events,
                                     &listModeTime);
