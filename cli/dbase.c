...
#define VERSION "0.2"
#define PROGRAM_NAME "digibase C-control program"
...
void print_usage() {
  ...
  printf("Usage: dbase \t[-d n][-t,i T] [-s n] [-lm [c/a/t]] [-l,d,q,h] [-o "
         "file]\n\t\t[-CTRL [on/off]]\n\t\t[-set hvt/fg/pw/ltp/rtp/gsch/zsch "
         "[value(s)]]\n\t\t[-clr [all/spec/pres/cnt]]\n");

  /* Argument desc.: */
  printf("\t -d n\tOperate on device with device_no n (see -l)\n");
  printf("\t -t\tSample for time T, then exit (-1=infinite)\n");
  printf("\t -i\tSampling interval T\n\t\t(pha-mode: default 1s, list-mode: "
         "default 50ms)\n\t\tvalid time suffix's are one of: ms,s,m,h,d,y "
  ...
  /* Clear commands */
  printf(" Clear: \n");
  printf("\t -clr all/spec/pres/cnt (all three or one of: "
         "spectrum/presets/counters)\n");
  ...
  printf("\t -set gsch C W\t (Gain stab center ch. and width)\n");
  ...
  printf(
      "\t -name DEV_NAME\t Name digibase with device_no n (use with -d n)\n");
  ...
  printf("\nExample 1:\t dbase -q -hv on -start -t 50\nwill enable hv, start "
         "measurement, measure for 50 seconds and then exit\nwithout printing "
         "anything other than ascii spectra to stdout.\n");
  printf("\nExample 2:\t dbase -q -hv on -lm c -t 1.0h -i 0.05 -o "
         "outp.txt\nwill enable hv, start measurement in list mode, measure "
         "for 1h and exit.\nThe counts recieved per 50ms by digibase will be "
         "printed to 'outp.txt'\n");
  printf("\n(C) Peder Kock, 2011,2012.\n\n");
}
...
int main(int argc, char **argv) {
  ...
  /* Arguments from user */
  /* General */
  int on = 0, stat = -1, d = 0, b = 0, ser = 0, spec = 0, cps = 0, list = 0,
      dev = 0, name = 0;
  /* ROI prints */
  int roi = 0;
  ...
  */
  for (k = 1; k < argc; k++) {
    ...
    /* List mode , with (at least one) parameter(s) [c/a/t] */
      ...
      if (lmc == 0 && lmt == 0 && lma == 0) {
        if (!q)
          fprintf(stderr,
                  "E: list mode arguments must be one or more of 'c,a,t'\n");
        return EXIT_FAILURE;
      }
      k++;
    }
  ...
  if (opts.haveopts == 0 && t == 0UL && on == 0 && stat == -1 && s == 0 &&
      hv == NULL && lm == 0 && ser == 0 && gs == NULL && zs == NULL &&
      spec == 0 && save == 0 && load == 0 && roi == 0 && list == 0 &&
      name == 0) {
    if (!q)
  ...
  if (name) {
    ...
    errc = mkdir(dir, S_IRUSR | S_IWUSR | S_IXUSR |     /* User perm,   7 */
    ...
    if (libdbase_get_spectrum(det) < 0) {
      ...
      if (d == 0) {
        ...
        if (b == 0)
          ...
          libdbase_print_diff_file_spectrum_binary(det,
                                                   fh == NULL ? stdout : fh);
      }
    }
  }
  ...
  if (roi == 1) {
    if (libdbase_get_spectrum(det) < 0) {
      ...
      uint roi_sum;
      libdbase_print_roi(det, roi_lc, roi_hc, &roi_sum,
                         fh == NULL ? stdout : fh);
  ...
  if (lm == 1 && sleept <= 1000000UL && sleept > 0UL && t > 0UL) {
    ...
    if (sleept == 1000000UL)
      sleept = 50000UL;
    if (!q)
      printf("Starting List Mode measurement freq: %llu Hz",
             1000000ULL / sleept);
    ...
    for (i = 0; i < cycles; i++) {
      ...
      if ((err = libdbase_get_spectrum(det)) >= 0) {
        if (cps == 1) {
          ...
          fprintf(fh == NULL ? stdout : fh, "%s%0.00f\n",
                  q ? "" : "CPS: ", cps);
          last_sum = sum;
        } else if (d == 0) {
        ...
        if (s > 0UL && (i + 1) % s == 0) {
          ...
          if (err >= 0) {
            if (b == 0)
              libdbase_print_file_status(&det->status, det->serial,
                                         fh == NULL ? stdout : fh);
            else {
              fwrite(&det->status, sizeof(status_msg), 1,
                     fh == NULL ? stdout : fh);
              fflush(fh == NULL ? stdout : fh);
            }
          } else if (!q) {
            fprintf(stderr, "E: couldn't read status (err=%d) - exiting\n",
                    err);
            break;
          }
        }
...
void measure_list_mode(char lmc, char lma, char lmt, unsigned long long lm_time,
  ...
  */
  ...
  if (lma == 1 || lmt == 1) {
    ...
    if (data == NULL) {
      fprintf(stderr, "dbase E: couln't allocate memory for listmode pulses\n");
      return;
    }
  }
  ...
  for (k = 0; k < e; k++) {
    ...
    if (data != NULL) {
      if (lma == 1 && lmt == 1)
        for (n = 0; n < read; n++)
          /* amp and time */
          fprintf(fh == NULL ? stdout : fh, "%u\t%u\n", data[n].amp,
                  data[n].time);
      else if (lma == 1)
        for (n = 0; n < read; n++)
          /* amp */
          fprintf(fh == NULL ? stdout : fh, "%u\n", data[n].amp);
      else
        for (n = 0; n < read; n++)
          /* time */
          fprintf(fh == NULL ? stdout : fh, "%u\n", data[n].time);
